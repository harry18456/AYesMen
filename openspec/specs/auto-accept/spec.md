...
Implementation uses direct gRPC calls to the Antigravity language server over HTTP/2:
...
All gRPC calls SHALL be sent via the HTTP/2 transport (`callGrpc()`), which maintains a persistent HTTP/2 session to the language server. Request bodies SHALL be JSON-encoded.
...
- **THEN** `HandleCascadeUserInteraction` is called over HTTP/2 with `confirm: true` and the command runs automatically

#### Scenario: No pending step
- **WHEN** polling loop runs but there are no pending agent steps
- **THEN** gRPC calls complete over HTTP/2 with no-op result and no error is surfaced
