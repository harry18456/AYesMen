...
function patchCodeiumWebview(context, forcePrompt = false) {
    ...
    if (!codeiumExt) {
        if (forcePrompt)
            vscode.window.showErrorMessage('[AYesMan] Antigravity/Codeium extension not found.');
        return;
    }
    ...
    if (!targetFile) {
        ...
        if (forcePrompt)
            vscode.window.showErrorMessage('[AYesMan] Could not find Codeium webview JS file.');
        return;
    }
    try {
        ...
        if (content.includes('__ayesman_injected')) {
            if (forcePrompt)
                vscode.window.showInformationMessage('[AYesMan] Patch already applied!');
            return;
        }
        ...
        if (forcePrompt)
            vscode.window.showErrorMessage('[AYesMan] Failed to patch file: ' + e.message);
    }
}
function deactivate() { }
