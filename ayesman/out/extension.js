...
function currentWorkspacePaths() {
    return new Set((vscode.workspace.workspaceFolders ?? []).map((f) => f.uri.fsPath.toLowerCase().replace(/\\/g, "/")));
}
...
async function tryAutoAcceptStep(server) {
    ...
    const candidates = Object.entries(summaries)
        .filter(([, s]) => {
        if (wsPaths.size === 0)
            return true; // no workspace open → accept all
        const cascadeWorkspaces = s?.workspaces ?? [];
