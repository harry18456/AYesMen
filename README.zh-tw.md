...
- **狀態列即時顯示**：右下角永遠顯示剩餘配額最低的模型與百分比（例如 `⚠ Gemini 3 Pro: 20%`）
- **色彩警示**：🟢 ≥80% · 🟡 40–79% · 🔴 <40%（背景變色）
- **懸浮提示**：滑鼠停留可查看所有模型百分比、重置時間、帳號 Prompt 與 Flow Credits
...
- **多專案安全**：同時開啟多個 Antigravity 視窗時，每個視窗只處理自己 workspace 的 Agent 對話
- **預設**：啟動時為 ON
...
PowerShell: Get-CimInstance Win32_Process (language_server_windows_x64.exe)
  ...
  → 對每個 port 送出 Heartbeat 請求，找到正確的 gRPC 端點
